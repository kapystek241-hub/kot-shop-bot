...
TEXT_DOCS = (
    ...
    "Для получения чека требуется написать в поддержку внутри Telegram-бота, раздел находится "
    "(Меню -> Поддержка -> Заполнение формы обращения). Чек предоставляется только по просьбе.\n\n"
    "Проект является коммерческим и не несёт ответственности в случае неправильно указанных данных "
    "при заполнении формы покупки. В случае если ошибка случилась и товар не доставлен, следует написать в поддержку.\n\n"
    "При проведении турниров от магазина KotShop241 участники, которые подтвердили участие, "
    "автоматически соглашаются с правилами, которые находятся в разделе "
    "(Меню -> Турнир -> Правила участия). Штрафы предусмотрены в том же разделе, в случае нарушения "
    "будут использованы санкции, которые указаны. Для подачи апелляции используйте контакты ниже.\n\n"
    "Магазин имеет полное право заблокировать использование сервиса в случае возникновения расследования "
    "по отношению к покупателю. В случае если вы не согласны с блокировкой, следует написать в поддержку "
    "по ссылкам ниже.\n\n"
...
TEXT_RAFFLE_INFO = (
    "Розыгрыш проходит на каналах в прямом эфире, информация о том, где проходит розыгрыш, "
    "ищите в телеграм-канале."
)
TEXT_CLAIM_WIN_INFO = (
    "Поздравляю с победой! Для получения выигрыша вы должны быть подписаны на телеграм-канал "
    "<a href='https://t.me/KotShop241'>телеграм-канал</a>, отправить ваш UID, который должен начинаться на 5."
)
TEXT_RAFFLE_DISABLED = (
    "Доброго времени суток, на данный момент розыгрыши не проводились. "
    "Следите за новостями в моем <a href='https://t.me/KotShop241'>телеграм-канале</a>, "
...
TEXT_TOURNAMENT_FORM = (
    ...
    "Подтверждение о том, что правила были прочитаны всеми участниками, вы являетесь подписанными "
    "на телеграм-канал аккаунтами, с которыми будет проводиться дальнейшее общение.\n\n"
...
def kb_raffle_claim():
    builder = InlineKeyboardBuilder()
    builder.button(text="1.3 Забрать выигрыш", callback_data="claim_win")
    builder.button(text="❌ Назад", callback_data="back_to_raffle")
    builder.adjust(2)
    return builder.as_markup()

def kb_tournament_apply():
    builder = InlineKeyboardBuilder()
    builder.button(text="1.4 Принять участие", callback_data="apply_tournament")
    builder.button(text="❌ Назад", callback_data="back_to_menu")
    builder.adjust(2)
    return builder.as_markup()
...
# Админ-команда для включения/выключения розыгрышей
@dp.message(Command("toggle_raffle"))
async def cmd_toggle_raffle(message: types.Message):
    global RAFFLE_ENABLED
    ...
    RAFFLE_ENABLED = not RAFFLE_ENABLED
    status_text = "✅ Розыгрыши ВКЛЮЧЕНЫ" if RAFFLE_ENABLED else "❌ Розыгрыши ВЫКЛЮЧЕНЫ"
...
async def cb_tournament_main(callback: types.CallbackQuery):
    await callback.message.edit_text(
        ...
        reply_markup=kb_tournament_apply(),
        parse_mode="HTML"
    )
    await callback.answer()
...
# Заглушки для кнопок «Акции» и «Поддержка»
@dp.callback_query(F.data.in_({"promo_main", "support_main"}))
async def cb_placeholders(callback: types.CallbackQuery):
    await callback.answer("Раздел в разработке", show_alert=True)

# Обработка обычного текста (для UID и т.п.) — только в контексте турнира
@dp.message()
async def handle_text(message: types.Message):
    # Если пользователь в процессе подачи заявки на турнир — можно тут ловить UID/данные.
    # Но в текущей реализации мы используем кнопки «Подтвердить», а не свободный ввод.
    # Здесь можно добавить логику, если позже захочешь принимать UID как текст.
    pass
